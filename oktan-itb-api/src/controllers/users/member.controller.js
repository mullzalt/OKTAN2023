...
const { Member, User, Competition } = require('../../models');
...
exports.getMembers = asyncHandler(async (req, res) => {
    ...
    var condition = where ? {
        [Op.or]: [
            ...
            { phone: { [Op.like]: `%${where}%` } },
            { '$user.username$': { [Op.like]: `%${where}%` } },
            { '$user.email$': { [Op.like]: `%${where}%` } },
        ]
    } : null;
    ...
    await Member.findAndCountAll({
        ...
        include: [{
            model: User,
            attributes: ['username', 'email', 'verified', 'id']
        }],
    })
        .then((data) => {
...
exports.getMemberById = asyncHandler(async (req, res) => {
    ...
    const member = await Member.findOne({
        where: { id: memberId },
        include: [
            { model: User, attributes: ['id', 'username', 'email', 'verified'] },
